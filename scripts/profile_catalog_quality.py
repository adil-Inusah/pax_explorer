...
def build_quality_report(
    *,
    ...
    gates = {
        ...
        "error_validation_count_zero": error_validation_count == 0,
    }
    ...
    return {
        ...
        "counts": {
            ...
            "orphan_relationship_sources": len(orphan_source_relationships),
            "invalid_relationship_target_links": len(invalid_target_links),
            "orphan_evidence_sources": len(orphan_evidence_sources),
            "blank_evidence_targets": len(blank_evidence_targets),
            "validated_relationships": validated_relationship_count,
            "relationships_without_linked_validation": len(missing_relationship_validation_ids),
            "orphan_validation_relationships": len(orphan_validation_relationships),
            "valid_validations": valid_validation_count,
            "tracked_nonerror_validations": tracked_nonerror_count,
            "error_validations": error_validation_count,
        },
        "distributions": {
...
def load_catalog(catalog_root: Path) -> dict[str, Any]:
    missing = [name for name in REQUIRED_FILES.values() if not (catalog_root / name).exists()]
    if missing:
        raise FileNotFoundError(
            "Required catalog files were not found: " + ", ".join(sorted(missing))
        )
    manifest = read_json(catalog_root / REQUIRED_FILES["manifest"])
    sources = read_json(catalog_root / REQUIRED_FILES["source_manifests"])
    if not isinstance(manifest, dict):
        raise ValueError("catalog_manifest.json must contain a JSON object.")
    if not isinstance(sources, dict):
        raise ValueError("catalog_source_manifests.json must contain a JSON object.")
    return {
        "manifest": manifest,
        "objects": record_list(read_json(catalog_root / REQUIRED_FILES["objects"]), "objects"),
        "relationships": record_list(read_json(catalog_root / REQUIRED_FILES["relationships"]), "relationships"),
        "evidence": record_list(read_json(catalog_root / REQUIRED_FILES["evidence"]), "evidence"),
        "validations": record_list(read_json(catalog_root / REQUIRED_FILES["validations"]), "validations"),
        "warnings": record_list(read_json(catalog_root / REQUIRED_FILES["warnings"]), "warnings"),
...
def main() -> int:
    ...
    try:
        ...
        print(f"Error validations              : {counts['error_validations']:,}")

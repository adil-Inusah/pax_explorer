...
def build_object_catalog(*, current_root: Path = CURRENT_ROOT, snapshot_root: Path = SNAPSHOT_ROOT,
    ...
    for declaration in declarations:
        process_id = clean(
            declaration.get("process_id")
            or f"process::{clean(declaration.get('process_name'))}"
        )
        shared_id = clean(
            declaration.get("shared_source_id")
            or declaration.get("source_id")
        )
        source_type = token(declaration.get("source_type"))
        source = shared_sources.get(shared_id, {})
        config_id = clean(
            declaration.get("configured_data_source_id")
            or configured_source_id(process_id, source_type, shared_id)
        )
        ...
        if shared_id in objects:
            add_relationship(
                config_id,
                "REFERENCES_SHARED_SOURCE",
                shared_id,
                "REFERENCE",
                generated=True,
            )

        # The process-specific declaration is authoritative. The shared source
        # record is only a backward-compatible fallback for older snapshots.
        effective_cube = clean(
            declaration.get("effective_cube_name")
            or declaration.get("cube_name")
            or declaration.get("server_source_name")
            or declaration.get("client_source_name")
            or source.get("effective_cube_name")
            or source.get("cube_name")
            or source.get("server_source_name")
            or source.get("client_source_name")
        )
        view_name = clean(
            declaration.get("view_name")
            or source.get("view_name")
        )
        dimension_name = clean(
            declaration.get("dimension_name")
            or source.get("dimension_name")
        )
        effective_hierarchy = clean(
            declaration.get("effective_hierarchy_name")
            or declaration.get("hierarchy_name")
            or source.get("effective_hierarchy_name")
            or source.get("hierarchy_name")
            or dimension_name
        )
        subset_name = clean(
            declaration.get("subset_name")
            or source.get("subset_name")
        )
        path_value = clean(
            declaration.get("server_source_name")
            or declaration.get("client_source_name")
            or source.get("server_source_name")
            or source.get("client_source_name")
        )
        signature = (
            source_type,
            normalize(effective_cube),
            normalize(view_name),
            normalize(dimension_name),
            normalize(effective_hierarchy),
            normalize(subset_name),
            normalize(path_value),
        )
        source_signatures[shared_id].add(signature)

        if source_type == "TM1_CUBE_VIEW":
            key = view_reference_key(effective_cube, view_name)
            ref_id = reference_id(key)
            resolution = resolver.resolve_view(effective_cube, view_name)
            add_object(
                ref_id,
                "TM1_VIEW_REFERENCE",
                "CONCEPTUAL_REFERENCE",
                {
                    "display_value": f"{effective_cube} / {view_name}",
                    "configured_cube_name": effective_cube,
                    "configured_view_name": view_name,
                    **resolution,
                },
                key,
            )
            add_relationship(
                ...
                "REFERENCE",
                generated=True,
                validation_status=resolution["resolution_status"],
            )
            resolutions.setdefault(
                ref_id,
                {"reference_object_id": ref_id, **resolution},
            )
            if resolution["resolved_target_object_id"]:
                add_relationship(
                    ref_id,
                    "RESOLVES_TO_VIEW",
                    resolution["resolved_target_object_id"],
                    "RESOLUTION",
                    generated=True,
                    validation_status=resolution["resolution_status"],
                )
            ...
            key = subset_reference_key(
                dimension_name,
                effective_hierarchy,
                subset_name,
            )
            ref_id = reference_id(key)
            resolution = resolver.resolve_subset(
                dimension_name,
                effective_hierarchy,
                subset_name,
            )
            add_object(
                ref_id,
                "TM1_SUBSET_REFERENCE",
                "CONCEPTUAL_REFERENCE",
                {
                    "display_value": (
                        f"{dimension_name} / "
                        f"{effective_hierarchy} / {subset_name}"
                    ),
                    "dimension_name": dimension_name,
                    "hierarchy_name": effective_hierarchy,

...
class LegacyHierarchyElementService:
    """Compatibility adapter for older TM1 clients and test doubles.

    Production TM1 connections should use the targeted
    ElementService.get_element_attributes() endpoint. This adapter is
    used only when that targeted service is unavailable.
    """
    ...
    def get_element_attributes(
        ...
        if not callable(get_hierarchy):
            raise AttributeError(
                "The fallback hierarchy service does "
                "not support hierarchy retrieval."
            )
...
def get_element_service(
    ...
    """Return the fastest available attribute-definition service.

    Preferred paths use the targeted TM1 ElementAttributes endpoint.
    The hierarchy adapter exists only for compatibility with older
    clients and unit-test doubles.
    """
    ...
    hierarchy_get = getattr(
        hierarchy_service,
        "get",
        None,
    )

    if callable(hierarchy_get):
        return LegacyHierarchyElementService(
            hierarchy_service
        )

    raise AttributeError(
        "The TM1 connection does not expose "
        "elements.get_element_attributes() "
        "or a compatible hierarchy service."
    )
...
def collect_attributes(
    ...
    *,
    ...
    for index, dimension_name in enumerate(dimension_names, start=1):
        ...
        if hierarchy_names and element_service is None:
            raise RuntimeError(
                "An element service is required when hierarchies are present."
            )
                ...
                hierarchy_metrics.append(
                    {
                        "dimension_name": dimension_name,
                        "hierarchy_name": hierarchy_name,
                        "hierarchy_list_seconds": round(
                            hierarchy_list_seconds, 6
                        ),
                        "attribute_request_seconds": round(
                            request_seconds, 6
                        ),
                        "attribute_count": (
                            len(records_by_key) - record_count_before
                        ),
                        "slow_request": (
    ...
    type_counts = Counter(
        record["attribute_data_type"] for record in records
    )
    ...
    metrics = {
        ...
        "mean_seconds_per_dimension": (
            round(total_seconds / len(dimension_names), 6)
            if dimension_names
            else 0.0
        ),
        "mean_attribute_request_seconds": (
            round(statistics.mean(request_durations), 6)
            if request_durations
            else 0.0
        ),
        "median_attribute_request_seconds": (
            round(statistics.median(request_durations), 6)
            if request_durations
            else 0.0
        ),
        ...
        "maximum_attribute_request_seconds": (
            round(max(request_durations), 6)
            if request_durations
            else 0.0
        ),
    ...
    if status == "COMPLETE" and publish_current:
        ...
        write_json(
            current_root / "attribute_collection_metrics.json", metrics
        )
        write_json(
            current_root / "attribute_collection_errors.json", errors
        )
    ...
    try:
        with get_tm1_connection() as tm1:
            ...
            if uses_default_options:
                manifest = collect_attributes(tm1)
            else:
